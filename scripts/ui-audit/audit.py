...
# Canon = Ariel's enterprise-blue palette, settled 2026-09-07 and unchanged since.
# It is the SAME 15 values enforced by biddeed-web/lib/design-tokens.ts and by
# scripts/ui-audit/palette_gate_worker.py -- those three lists must never disagree.
# The evaluator must match the production design contract; it must NEVER be widened
# to accept the retired cream/terracotta house brand (#f5f0e8 #fbfaf7 #f8d4c5
# #1f1b16 #ddd5c9 #c15f3c #a94d30 #8f4028 #7a3424) or the superseded blues
# (#0073cf #005daa #002a54 #e8f4fc) merely to reduce the red count. Widening this
# set is how cream shipped to production on 2026-09-08 with CI green.
...
JS_SWEEP = r"""() => {
  ...
  function inScroller(el){let e=el.parentElement;while(e&&e!==document.documentElement){const c=getComputedStyle(e);if(/(auto|scroll)/.test(c.overflowX+' '+c.overflow))return true;e=e.parentElement}return false}
  function sel(el){var c=(typeof el.className==='string'?el.className:'').trim().split(/\s+/).filter(Boolean).slice(0,2).join('.');return el.tagName.toLowerCase()+(c?'.'+c:'')}
  const vw=window.innerWidth; const docOverflow=document.documentElement.scrollWidth>vw+1; let textOverflow=0, offscreen=0, smallTap=0, tiny=0, offscreenWhere=[], smallTapWhere=[], tinyWhere=[];
  for(const el of document.querySelectorAll('body *')){const cs=getComputedStyle(el); if(cs.display==='none'||cs.visibility==='hidden')continue; const r=el.getBoundingClientRect(); if(!r.width||!r.height)continue; const scrollable=/(auto|scroll)/.test(cs.overflowX+cs.overflow); const hidden=/hidden/.test(cs.overflowX+cs.overflow); const t=(el.innerText||'').trim();
    ...
    if(r.right>vw+2&&r.width>40&&!scrollable&&!inScroller(el)){offscreen++;if(offscreenWhere.length<6)offscreenWhere.push(sel(el)+'@r'+Math.round(r.right))}
    if((el.tagName==='A'||el.tagName==='BUTTON')&&t&&(r.width<32||r.height<32)){smallTap++;if(smallTapWhere.length<6)smallTapWhere.push(sel(el)+'@'+Math.round(r.width)+'x'+Math.round(r.height))}
    if(/^(P|LI|DD)$/.test(el.tagName)&&t.length>20&&parseFloat(cs.fontSize)<(vw<600?16:15)){tiny++;if(tinyWhere.length<6)tinyWhere.push(sel(el)+'@'+cs.fontSize)}}
        ...
        for vw,vh in ((1440,900),(390,844)):
            ...
            for route in routes:
                ...
                gates={
                 ...
                 # A page that declares noindex is deliberately not for search -- the
                 # key-gated /report/ pages must never be indexed, so canonical/meta
                 # rules do not apply to them. This is an exemption for pages that opt
                 # OUT of search, never a way to skip the gate: drop the noindex and the
                 # full SEO check applies again.
                 "SEO": ("noindex" in (r.get("robots","") or "").lower()) or (len(r.get("h1",[]))==1 and 0<len(r.get("meta",""))<=155 and bool(r.get("canonical"))),

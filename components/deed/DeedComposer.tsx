...
export default function DeedComposer({
  ...
  return (
    ...
    >
      {notice.length > 0 ? (
        <ul className="mb-2 space-y-1 rounded-md border border-primary/40 bg-secondary px-3 py-2">
          {notice.map((n) => (
        ...
        <>
          <ul className="mb-2 flex flex-wrap gap-2">
            {files.map((f) => (
              <li
                key={f.id}
                className="flex max-w-full items-center gap-2 rounded-md border border-border bg-secondary py-1 pl-2 pr-1"
              >
                {f.previewUrl ? (
                ...
                <span className="min-w-0">
                  <span className="block max-w-[10rem] truncate text-xs text-foreground">
                    {f.name}
                  </span>
      ...
      >
        ...
        <textarea
          ...
          className="block max-h-[200px] w-full resize-none bg-transparent px-3 pb-1 pt-3 text-sm text-foreground outline-none placeholder:text-muted-foreground"
        />

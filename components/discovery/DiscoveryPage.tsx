...
export default function DiscoveryPage() {
  ...
  return (
    <section className="min-h-full bg-[var(--canvas,#f7f1e8)] px-4 py-8 text-[var(--ink,#1f1b16)] sm:px-8 lg:px-12">
      <div className="mx-auto max-w-6xl">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 max-w-2xl text-base leading-7 text-[var(--muted,#766f67)]">Search BidDeed’s source-backed auction inventory by location or case. Results are bounded, freshness-labeled, and never presented as legal, financial, or investment advice.</p>
        </div>
        ...
        <form onSubmit={submit} className="mt-8 border-y border-black/10 py-5" aria-label="Auction discovery search">
          <div className="flex flex-col gap-3 lg:flex-row">
            <label className="flex min-w-0 flex-1 items-center gap-3 border border-black/15 bg-white/70 px-4 py-3 focus-within:border-[var(--terracotta,#c15f3c)]">
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} maxLength={80} className="min-w-0 flex-1 bg-transparent text-base outline-none placeholder:text-[var(--muted,#766f67)]" placeholder="Address, city, ZIP, parcel, case number…" />
            </label>
            <label className="border border-black/15 bg-white/70 px-3 py-2 text-sm lg:w-48">
              <span className="block text-[11px] font-bold uppercase tracking-wider text-[var(--muted,#766f67)]">County slug</span>
            ...
            <label className="border border-black/15 bg-white/70 px-3 py-2 text-sm lg:w-48">
              <span className="block text-[11px] font-bold uppercase tracking-wider text-[var(--muted,#766f67)]">Sale type</span>
          ...
          <p className="mt-3 text-sm text-[var(--muted,#766f67)]" role="status">{message}</p>
        </form>

        <div className="grid gap-4 border-b border-black/10 py-6 sm:grid-cols-3">
          <div className="flex items-start gap-3"><MapPinned className="mt-0.5 h-5 w-5 text-[var(--terracotta,#c15f3c)]" aria-hidden="true" /><div><p className="text-sm font-bold">Coverage</p><p className="mt-1 text-sm text-[var(--muted,#766f67)]">{coverageLabel}</p></div></div>
          <div className="flex items-start gap-3"><CalendarDays className="mt-0.5 h-5 w-5 text-[var(--terracotta,#c15f3c)]" aria-hidden="true" /><div><p className="text-sm font-bold">Upcoming scope</p><p className="mt-1 text-sm text-[var(--muted,#766f67)]">{summary?.upcoming?.toLocaleString('en-US') ?? '—'} scheduled records</p></div></div>
          <div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 text-[var(--terracotta,#c15f3c)]" aria-hidden="true" /><div><p className="text-sm font-bold">Evidence rule</p><p className="mt-1 text-sm text-[var(--muted,#766f67)]">Every result retains its source link.</p></div></div>
        </div>

        {searched && rows.length > 0 && <div className="mt-8 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><caption className="mb-3 text-left text-lg font-bold">Upcoming source-backed inventory</caption><thead className="border-b border-black/15 text-xs uppercase tracking-wider text-[var(--muted,#766f67)]"><tr><th className="py-3 pr-4">Property</th><th className="py-3 pr-4">County</th><th className="py-3 pr-4">Date</th><th className="py-3 pr-4">Sale</th><th className="py-3 pr-4">Opening bid</th><th className="py-3">Source</th></tr></thead><tbody>{rows.map((row) => <tr key={String(row.id)} className="border-b border-black/10"><td className="py-4 pr-4"><div className="font-semibold">{row.property_address || 'Address not published'}</div><div className="mt-1 text-xs text-[var(--muted,#766f67)]">{row.city || '—'} · {row.case_number || 'Case unavailable'}</div></td><td className="py-4 pr-4">{row.county || '—'}</td><td className="py-4 pr-4">{row.auction_date || '—'}</td><td className="py-4 pr-4">{row.sale_type || '—'}</td><td className="py-4 pr-4">{money(row.opening_bid)}</td><td className="py-4">{row.source_url ? <a className="font-semibold text-[var(--terracotta,#c15f3c)] underline" href={row.source_url} target="_blank" rel="noreferrer">View source</a> : <span className="text-[var(--muted,#766f67)]">Unavailable</span>}</td></tr>)}</tbody></table></div>}
        {searched && rows.length === 0 && <div className="mt-8 flex items-start gap-3 border border-black/10 bg-white/45 px-5 py-4 text-sm text-[var(--muted,#766f67)]"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-[var(--terracotta,#c15f3c)]" aria-hidden="true" /><p>No result table is shown without source-backed records. Coverage varies by county and sale type; confirm material facts with the relevant clerk or official source.</p></div>}
      </div>
    </section>
  )
}
